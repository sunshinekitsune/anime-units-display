...
function showDialogue(text, duration) {
    ...
    hintLabel.actions(
        ...
        Actions.run(() =>
            hintLabel.actions(
                Actions.parallel(
                    Actions.alpha(0, 0.8, Interp.smooth),
                    Actions.translateBy(0, -30, 0.8, Interp.sinkIn),
                ),
                Actions.hide(),
            ),
        ),
    ...
    Core.app.post(() => {
        ...
        for (let i = 0; i < waterFloors.size; i++) {
            if (Vars.indexer.isBlockPresent(waterFloors.get(i))) {
                ...
                if (
                    (f instanceof ShallowLiquid &&
                        summerFloors.contains(f.floorBase)) ||
                    f.drownTime > 0
                ) {
                    baseSummerScore++;
                }
        ...
        for (
            let i = 0;
            i < summerFloors.size && baseSummerScore < SUMMER_MINIMUM;
            i++
        ) {
...
Events.on(
    UnitControlEvent,
    (e) => e.player == Vars.player && showKey("control", 50),
);
Events.on(
    PayloadDropEvent,
    (e) => e.carrier == Vars.player.unit() && showKey("drop", 50),
);
Events.on(
    PickupEvent,
    (e) => e.carrier == Vars.player.unit() && showKey("pickup", 0),
);
...
Events.run(ClientLoadEvent, () => {
    ...
    Vars.ui.settings.addCategory(
        "Anime Units Display",
        Core.atlas.drawable("alphaaaa"),
        (t) => {
            t.check(
                "Enable global override",
                Core.settings.getBool(KEY_GLOBAL_OVERRIDE_ENABLED, false),
                (b) => Core.settings.put(KEY_GLOBAL_OVERRIDE_ENABLED, b),
            )
                .left()
                .row();
            t.check(
                "Use fallback handling",
                Core.settings.getBool(KEY_FALLBACK_HANDLING, true),
                (b) => Core.settings.put(KEY_FALLBACK_HANDLING, b),
            )
                .left()
                .row();
            const overrideButton = t
                .button(
                    "Texture: " +
                        Core.settings.getString(
                            KEY_GLOBAL_OVERRIDE_TEXTURE,
                            DEFAULT_OVERRIDE_CHARACTER,
                        ),
                    () => {
                        ...
                        const baseNames = [];
                        const chibisDirectory =
                            mod.root.child(CHIBIS_DIRECTORY);
                        if (chibisDirectory.exists()) {
                            chibisDirectory.list().forEach((f) => {
                                const n = f.nameWithoutExtension();
                                if (
                                    !n.match(
                                        /-(mining|building|shooting|summer|winter)$/,
                                    )
                                ) {
                                    baseNames.push(n);
                                }
    ...
    const displayElement = extend(Element, {
        draw() {
            ...
            const unit = Vars.player.unit();
            const isFallback = Core.settings.getBool(
                KEY_FALLBACK_HANDLING,
                true,
            );
            ...
            if (unit && !Vars.player.dead()) {
                if (unit.mining()) {
                    activeTex =
                        charData.mining ||
                        (isFallback ? charData.shooting : null) ||
                        charData.main;
                    ...
                    activeTex =
                        charData.shooting ||
                        (isFallback ? charData.mining : null) ||
                        charData.main;
                }
            }
            ...
            const anchor = lastType
                ? !NO_ANCHOR_CHARACTERS.includes(lastType.name)
                : true;
            const fin = Interp.swingOut.apply(fade);

            const squishFactor =
                0.2 * squish + Mathf.sin(Time.globalTime, 20, 0.01);
    ...
    displayElement.update(() => {
        ...
        if (!Vars.player.dead()) {
            ...
            const isGlobal = Core.settings.getBool(
                KEY_GLOBAL_OVERRIDE_ENABLED,
                false,
            );
            const targetChar = isGlobal
                ? Core.settings.getString(
                      KEY_GLOBAL_OVERRIDE_TEXTURE,
                      DEFAULT_OVERRIDE_CHARACTER,
                  )
            ...
            if (lastTargetChar != targetChar) {
                ...
                if (fade <= 0.01) {
                    ...
                    reloads = unit.mounts
                        ? Array(unit.mounts.length).fill(0)
                        : [];
                    squish = 1;
                    lastType = next;
                }
